...
type CarouselFieldValue = string | number | boolean | undefined | null;

type Column<T> = {
  key: keyof T | string;
  header: string;
  render?: (value: CarouselFieldValue, row: T) => ReactNode;
  sortable?: boolean;
  width?: string;
};
...
export const CarouselColumns = (): Column<Carousel>[] => [
  {
    ...
    render: (value: CarouselFieldValue, carousel: Carousel) => (
  ...
  },
  {
    key: "targetUrl",
    header: "Target URL",
    render: (value: CarouselFieldValue) => (
      typeof value === "string" && value ? (
        <a
          href={value}
          target="_blank"
