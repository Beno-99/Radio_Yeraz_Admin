...
export default function AdStats({ ads }: AdStatsProps) {
  const stats = useMemo(() => {
    ...
    const mobileAds = ads.filter((a) => a.platform === "mobile").length;
    const webAds = ads.filter((a) => a.platform === "web").length;
    const scheduledAds = ads.filter((a) => {
      if (!a.startDate) return false;
      const start = new Date(a.startDate);
      return a.isActive && start > new Date();
    }).length;
  ...
  const mainStats = [
    {
      ...
      subtitle: `Out of ${ads.length} total`,
      icon: Target,
